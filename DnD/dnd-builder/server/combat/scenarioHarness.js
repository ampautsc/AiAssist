...
const { runEncounter } = require('./engine/encounterRunner');
const { makeTacticalAI, makeDecision } = require('./ai/tactics');


// ═══════════════════════════════════════════════════════════════════════════
// MONSTER → AI PROFILE MAPPING
// ═══════════════════════════════════════════════════════════════════════════
 ...
 * 
 * @param {Object} build — populated build document
 * @param {Object} scenario — a SCENARIOS entry
 * @returns {Object[]} array of combat-ready creatures
 */
function createScenarioCombatants(build, scenario) {
 ...
 * @param {number} [options.numRuns=100] — number of simulations
 * @param {number} [options.logRuns=3] — how many runs to save full combat logs for
 * @param {boolean} [options.verbose=false] — log each encounter
 * @returns {Object} aggregated results
 */
function simulateScenario(build, scenario, options = {}) {
  ...
  for (let i = 0; i < numRuns; i++) {
    ...
    // Build profile resolver: bard → lore_bard, enemies → their tagged profile
 ...
 * 
 * @param {Object} build — populated build document
 * @param {Object} [options] — same as simulateScenario
 * @returns {Object[]} array of per-scenario aggregated results
 */
function simulateAllScenarios(build, options = {}) {
