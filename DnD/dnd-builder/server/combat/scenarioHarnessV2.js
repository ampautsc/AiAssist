/**
 * Scenario Simulation Harness — v2 (Immutable Zero-Trust Engine)
 *
...
const { runEncounter } = require('./engine-v2/EncounterRunner')
const { makeAdaptedAI, makeAdaptedReactionAI } = require('./engine-v2/TacticsAdapter')
const { SCENARIOS, MONSTER_PROFILES } = require('./scenarioHarness')
 ...
 */
function createScenarioState(build, scenario) {
...
function simulateScenario(build, scenario, options = {}) {
  ...
  for (let i = 0; i < numRuns; i++) {
    ...
    if (i < logRuns) {
      ...
      // v2 uses 'snapshots', v1 uses 'positionSnapshots' — expose both
