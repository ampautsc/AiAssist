...
const { createCreature, getTemplateKeys } = require('./data/creatures')
const { getSpell } = require('./data/spells')
const { processStartOfTurn, processEndOfTurnSaves } = require('./engine-v2/EncounterRunner')
...
function submitChoice(sessionId, choice) {
  ...
  try {
    const CombatNarrator = require('../services/CombatNarratorService')
    ...
    CombatNarrator.processStateTransition(
      sessionId,
      session.state, // Use current state as "prev" approximation
      session.state,
      activeId,
      resolution.result
    ).then(narrations => {
...
module.exports = {
  ...
  _extractRolls: extractRolls,
  _checkVictory: checkVictory,
  _processNewDeaths: processNewDeaths,
  _stopCleanup: stopCleanup,
}
