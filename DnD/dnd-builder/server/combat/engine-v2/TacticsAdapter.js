/**
 * TacticsAdapter — Bridge between v1 AI tactics and v2 menu-based engine.
 *
 ...
 *
 * Exports:
 *   adaptDecision(state, combatantId, v1Decision) → v2 choice | null
 *   makeAdaptedAI(profileMap) → getDecision(state, combatantId) for EncounterRunner v2
 */
 ...
 */
function matchAction(options, v1Action, state) {
  ...
  // ── Spell (via .spell field — covers both explicit type and implicit)
 ...
 */
function matchSpellOption(options, spellName, v1Action, state) {
  ...
  if (match.targetType === 'single') {
    if (targetId) {
      ...
      // v1 didn't specify a target — auto-select from valid targets
      choice.targetId = match.validTargets[0].id
    }
 ...
 */
function matchBonusAction(options, v1BonusAction, state) {
 ...
 */
function matchMovement(movementOptions, v1Movement, actorId, state) {
 ...
 * the movement → action sequence in EncounterRunner.
 *
 ...
 */
function adaptDecision(state, combatantId, v1Decision) {
  if (!v1Decision) return null

  const result = {}
  let hasAny = false

  // ── Map movement first (needed to project post-move state) ────────────
  ...
  // ── Fallback: if v1 gave a main action but we couldn't match, try dodge
 ...
 * @returns {Function} getDecision(state, combatantId) → v2 choice | null
 */
function makeAdaptedAI(profileMap) {
 ...
 *
 * @param {object|Function} profileMap - { [creatureId]: profileKey } or (creature) => profileKey
 * @returns {Function} getReaction(state, reactorId, event) → reaction | null
 */
function makeAdaptedReactionAI(profileMap) {
...
module.exports = {
  adaptDecision,
  makeAdaptedAI,
  makeAdaptedReactionAI,
  // Internal matchers exported for testing
  _matchAction: matchAction,
  _matchBonusAction: matchBonusAction,
  _matchMovement: matchMovement,
  _matchSpellOption: matchSpellOption,
}
