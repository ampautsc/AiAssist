/**
 * EncounterRunner v2 — Immutable, zero-trust encounter loop.
 *
 * Orchestrates the full combat encounter:
 *   1. Initiative roll
 *   2. Turn loop: reset → start-of-turn effects → decision → validate+resolve → end-of-turn saves → victory check
 *   3. Analytics
 *
 * Design:
 ...
 *       • Paralyzed + flying → fall damage
 *       • Gem Flight timer decrement
 *       • Concentration duration decrement
 *       • End-of-turn saves (Hold Person, Dragon Fear)
 *
 * Exports:
 *   rollInitiative(state) → GameState with initiative order
 *   resetTurnState(state, id) → GameState
 *   processStartOfTurn(state, id) → { state, skipped }
 *   processEndOfTurnSaves(state, id) → GameState
 *   checkVictory(state) → { over, winner }
 *   buildAnalytics(state) → analytics[]
 *   runEncounter({ state, getDecision, maxRounds }) → result
 */
 ...
 *   - enemy_attack_roll: an attack hit → Cutting Words / Silvery Barbs can reduce the roll
 *   - enemy_casting_spell: a spell was cast → Counterspell can negate it
 *
 * @param {GameState} state
 * @param {string}    actorId     - who just acted
 * @param {object}    actionResult - { type, hit?, roll?, targetAC?, spellName?, ... }
 * @param {Function}  getReaction  - (state, reactorId, event) → reaction | null
 * @returns {GameState}
 */
function processReactions(state, actorId, actionResult, getReaction, preSpellState) {
  ...
  // ── Reaction to Multiattack — process each sub-attack ────────────────
  ...
  // ── Reaction to Movement — Opportunity Attacks ─────────────────────────
  ...
  // ── Reaction to Spell Casting — Counterspell ─────────────────────────
 ...
 * @returns {GameState}
 */
function applyReaction(state, reactorId, actorId, reaction, event, preSpellState) {
  ...
  switch (reaction.type) {
    case 'cutting_words': {
      ...
      if (effectiveRoll < event.targetAC) {
        // Attack now misses — undo the damage
        ...
        currentState = currentState.withLog(
          `  ⚡ ${reactor.name} uses Cutting Words! ${biDie} roll: ${reduction}. ` +
          `Attack roll ${event.roll} → ${effectiveRoll} vs AC ${event.targetAC} — now MISSES!`
        )
      } else {
        currentState = currentState.withLog(
          `  ⚡ ${reactor.name} uses Cutting Words! ${biDie} roll: ${reduction}. ` +
          `Attack roll ${event.roll} → ${effectiveRoll} vs AC ${event.targetAC} — still hits.`
        )
      }
      break
    }
    ...
    case 'counterspell': {
      ...
      // otherwise requires ability check. In our sim most enemy spells are ≤ 3rd level.
      ...
      if (success) {
        // The spell is countered — revert ALL spell effects cleanly.
        ...
        if (preSpellState) {
          ...
          reverted = reverted.withLog(
            `  ⚡ ${reactor.name} casts Counterspell (${slotLevel}${ordinalSuffix(slotLevel)} level)! ` +
        ...
        if (caster.concentrating === event.spell) {
          const breakResult = ActionResolver._breakConcentration(currentState, actorId)
        ...
        currentState = currentState.withLog(
          `  ⚡ ${reactor.name} tries Counterspell but fails the check!`
        )
      }
      break
    }
    ...
    case 'silvery_barbs': {
      ...
      if (event.type === 'enemy_attack_roll') {
        // Force attacker to reroll attack — use a new d20 roll
        ...
        if (rerollTotal < event.targetAC) {
          ...
          currentState = currentState.withLog(
            `  ⚡ ${reactor.name} casts Silvery Barbs! Forced reroll: d20:${reroll} total ${rerollTotal} ` +
            `vs AC ${event.targetAC} — now MISSES!`
          )
        } else {
          currentState = currentState.withLog(
            `  ⚡ ${reactor.name} casts Silvery Barbs! Forced reroll: d20:${reroll} total ${rerollTotal} ` +
            `vs AC ${event.targetAC} — still hits.`
          )
        }
      }
      break
    ...
    case 'opportunity_attack': {
      ...
      // We use _resolveAttack which sets usedAction:true — we restore it afterward.
      ...
      currentState = currentState.withLog(
        `  ⚡ ${reactor.name} makes an opportunity attack as ${oaTarget.name} leaves their reach!`
      )
      ...
      const atkRes = ActionResolver._resolveAttack(currentState, reactorId, attackOpt)
      // Restore usedAction — OA spends the reaction, not the action
 ...
 * @returns {GameState}
 */
function resetTurnState(state, id) {
  const creature = state.getCombatant(id)
  // Note: vm_disadvantage is NOT cleared here — it persists until the creature
 ...
 */
function processStartOfTurn(state, id) {
  let currentState = state
  const creature = currentState.getCombatant(id)
  const logs = []

  // 1. Paralyzed + flying → fall
  ...
  if (updatedCreature.concentrating && updatedCreature.concentrationRoundsRemaining > 0) {
    ...
    if (remaining <= 0) {
      ...
      const breakResult = ActionResolver._breakConcentration(currentState, id)
      currentState = breakResult.state
 ...
 * @returns {GameState}
 */
function processEndOfTurnSaves(state, id) {
  let currentState = state
  const creature = currentState.getCombatant(id)
  const logs = []

  // Paralyzed → WIS save vs Hold Person
  if ((creature.conditions || []).includes('paralyzed')) {
    ...
    if (caster) {
      ...
      if (roll >= dc) {
        ...
        logs.push(`${creature.name} WIS save vs Hold Person: [d20:${raw}+${mod}=${roll} vs DC ${dc}] SUCCESS — no longer paralyzed.`)
      } else {
        logs.push(`${creature.name} WIS save vs Hold Person: [d20:${raw}+${mod}=${roll} vs DC ${dc}] FAIL — still paralyzed.`)
      }
    }
  }

  // Frightened → WIS save vs Dragon Fear
  const updatedCreature = currentState.getCombatant(id)
  if ((updatedCreature.conditions || []).includes('frightened')) {
    ...
    if (source && source.dragonFear) {
      ...
      if (roll >= dc) {
        ...
        currentState = currentState.withUpdatedCombatant(id, { conditions: newConditions })
        logs.push(`${updatedCreature.name} WIS save vs Dragon Fear: [${roll} vs DC ${dc}] SUCCESS — no longer frightened.`)
      } else {
        logs.push(`${updatedCreature.name} WIS save vs Dragon Fear: [${roll} vs DC ${dc}] FAIL — still frightened.`)
      }
    }
  }
 ...
 * @param {Function}    options.getDecision  - (state, combatantId) → choice object { optionId, targetId?, ... }
 * @param {Function}    [options.getReaction] - (state, reactorId, event) → reaction | null
...
function runEncounter(options) {
  ...
  while (state.round < maxRounds && !winner) {
    ...
    for (const actorId of state.initiativeOrder) {
      ...
      if (TurnMenu._isIncapacitated(postSotActor)) {
        state = state.withLog(`${postSotActor.name} is incapacitated — skipping turn.`)
  ...
  state = state.withLog(`\n=== ENCOUNTER ENDED — Winner: ${winner.toUpperCase()} after ${state.round} rounds ===`)
