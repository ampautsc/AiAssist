/**
 * ActionResolver — Zero-trust action execution for engine-v2.
 *
 ...
 *
 * Design:
 *   - Every resolve function takes GameState → returns { state: GameState, result }
 ...
 *   attack      → roll attack → roll damage → apply → concentration check
 *   multiattack → repeat attack N times
 *   spell       → spend slot → concentration switch → targeting → attack/save → damage → effects
 *   dodge       → add dodging condition
 *   dash        → double remaining movement
 *   disengage   → set disengage flag
 *   move        → update position + deduct movement
 *   hold        → no-op
 *   bardicInspiration → grant die to ally
 *   gemFlight   → activate flight
 *   endTurn     → no-op (EncounterRunner handles turn transition)
 */
...
const { resolveAoETargets } = require('../engine/targetResolver')
const TurnMenu = require('./TurnMenu')

// Effects that trigger Dark Devotion advantage on saves
const CHARM_FEAR_EFFECTS = new Set(['charmed_hp', 'charmed', 'frightened'])

// Conditions that cause auto-fail on STR/DEX saves (D&D 5e PHB)
const AUTO_FAIL_STR_DEX_CONDITIONS = new Set(['paralyzed', 'stunned', 'unconscious'])
 ...
 * Immunity → 0 damage.  Resistance → half damage (rounded down).
 */
 ...
 */
function checkPolymorphRevert(state, targetId, overkillDamage = 0) {
  ...
  // the spell ends when the beast form is destroyed — clear concentration.
  ...
  logs.push(`  → ${target.name}'s ${beastName} form is destroyed! Reverts to original form. HP: ${restoredHP}/${pre.maxHP}${carryover > 0 ? ` (${carryover} excess damage carried over)` : ''}`)
...
function removeCharmedHPOnDamage(state, targetId, damage) {
  ...
  if (newConditions.includes('charmed_hp')) {
    ...
    logs.push(`    → ${target.name} takes damage — Hypnotic Pattern charm breaks!`)
  }
  ...
  if (newConditions.includes('asleep')) {
    ...
    logs.push(`    → ${target.name} takes damage — wakes from Sleep!`)
  }
 ...
 */
function resolve(state, combatantId, choice) {
 ...
 * E.g. T-Rex bit the only enemy — no Tail target exists, so action is spent.
 */
function finalizeMultiattackAction(state, attackerId) {
  ...
  for (const w of multiattackWeaponsOnly) {
    ...
    for (const enemy of enemies) {
      ...
      const dist = TurnMenu._combatDistance(actor, enemy)
      if (dist <= weaponRange) return state // valid target exists — don't consume
    }
  }

  // No valid targets for remaining weapons — consume the action
  return state.withUpdatedCombatant(attackerId, { usedAction: true })
}

function resolveAttack(state, attackerId, option) {
  ...
  const within5ft = TurnMenu._combatDistance(attacker, target) <= 5
  ...
  // ── Distinct-weapon multiattack action economy ───────────────────────────
  ...
  // Same-weapon multiattack (e.g. Brute: Greataxe ×2) is handled by the
...
function resolveMultiattack(state, attackerId, option) {
  const attacker = state.getCombatant(attackerId)
  ...
  for (let i = 0; i < option.attackCount; i++) {
    ...
    if (!target || target.currentHP <= 0) {
      ...
      let closestDist = TurnMenu._combatDistance(curAttacker, closest)
      for (const e of livingEnemies) {
        const d = TurnMenu._combatDistance(curAttacker, e)
      ...
      if (mustAvoidFirstTarget) {
        logs.push(`  → ${target.name} is next target for Tail (cannot target the same creature as Bite)`)
      } else {
        logs.push(`  → ${target.name} is next target (redirect)`)
      }
    }

    if (mustAvoidFirstTarget && targetId === firstTrexTargetId) {
      logs.push(`  ${attacker.name} cannot use Tail on the same target as Bite — no valid second target in range.`)
      break
    }
    ...
    const isTargetFaerieFired = (target.conditions || []).includes('faerie_fire')
    const curAttacker = currentState.getCombatant(attackerId)
    const within5ft = TurnMenu._combatDistance(curAttacker, target) <= 5
...
function resolveSpell(state, casterId, option, choice) {
...
function resolveSingleTargetSpell(state, casterId, spellDef, slotLevel, targetId, parentLogs, choice) {
  ...
  if (HUMANOID_ONLY_SPELLS.has(spellDef.name) && target.type && target.type !== 'humanoid') {
    logs.push(`  → ${target.name} is not humanoid (${target.type}) — ${spellDef.name} has no effect!`)
    return { state: currentState, logs, result: {} }
  }

  // ── Polymorph: special stat-replacement mechanic ─────────────────────
  if (spellDef.name === 'Polymorph') {
    return resolvePolymorphSpell(currentState, casterId, targetId, spellDef, slotLevel, logs, choice)
  }

  // ── Magic Missile: auto-hit multi-dart spell ────────────────────────────
  if (spellDef.special && Array.isArray(spellDef.special) && spellDef.special.includes('auto_hit')) {
    ...
    logs.push(`  → Magic Missile: ${darts} darts, ${totalDamage} total ${spellDef.damage?.type || 'force'} damage.`)
    return {
      state: currentState,
      logs,
      result: {
  ...
  // ── Power Word Stun: auto-stun if target HP ≤ 150 ───────────────────────
  if (spellDef.special && Array.isArray(spellDef.special) && spellDef.special.includes('hp_threshold_150')) {
    ...
    if (freshTarget.currentHP <= 150) {
      ...
      logs.push(`  → ${freshTarget.name} is stunned! (${freshTarget.currentHP} HP ≤ 150)`)
    } else {
      logs.push(`  → ${freshTarget.name} has too many HP (${freshTarget.currentHP} > 150) — Power Word Stun fails!`)
    }
    return { state: currentState, logs, result: {} }
  }
  ...
  if (spellDef.healing) {
    ...
    logs.push(`  → Heals ${target.name} for ${healAmount}. HP: ${newHP}/${target.maxHP}`)
  ...
  if (spellDef.attack) {
    ...
    if (atkResult.hits) {
      ...
      logs.push(`  → Attack roll: [d20:${atkResult.natural}+${caster.spellAttackBonus}=${atkResult.total} vs AC ${target.ac}] HIT! ${dmgResult.damage} damage.`)
  ...
  if (spellDef.save) {
    ...
    if (saveResult.success) {
      logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] SUCCESS!${hasAdv ? ' (advantage)' : ''}`)
      if (!spellDef.save.negatesAll && spellDef.damage) {
        ...
        logs.push(`    → Half damage: ${dmgResult.damage}`)
        logs.push(...dmgResult.logs)
      }
    } else {
      ...
      if (lr.used) {
        ...
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] SUCCESS (Legendary Resistance)`)
        if (!spellDef.save.negatesAll && spellDef.damage) {
          const dmgResult = applySpellDamage(currentState, casterId, targetId, spellDef, slotLevel, false, 0.5)
          currentState = dmgResult.state
          spellDamage += dmgResult.damage
        ...
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] FAIL!`)
        if (spellDef.damage) {
          ...
          if (Array.isArray(dmgResult.damageRolls)) damageRolls.push(...dmgResult.damageRolls)
          damageDice = dmgResult.damageDice || damageDice
          logs.push(`    → ${dmgResult.damage} damage.`)
          logs.push(...dmgResult.logs)
        }
        // Apply conditions
        currentState = applySpellEffects(currentState, targetId, spellDef, logs)
  ...
  // ── Buff/utility spells with no attack/save/healing ──────────────────────
...
function resolveAreaSpell(state, casterId, spellDef, slotLevel, aoeCenter, parentLogs) {
  ...
  logs.push(`  → AoE hits ${targets.length} target(s)`)

  const dc = caster.spellSaveDC || 10

  // ── Sleep: HP-pool mechanic (no save) ────────────────────────────────
  ...
  for (const virtualTarget of targets) {
    ...
    if (spellDef.save) {
      ...
      if (saveResult.success) {
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: SUCCESS`)
        if (!spellDef.save.negatesAll && spellDef.damage) {
 ...
 */
function resolvePolymorphSpell(state, casterId, targetId, spellDef, slotLevel, parentLogs, choice) {
  const caster = state.getCombatant(casterId)
  const target = state.getCombatant(targetId)
  const logs = []
  let currentState = state
  ...
  if (!isSelfTarget && spellDef.save) {
    ...
    if (!saveResult.success) {
      ...
      currentState = lr.state
      if (lr.used) {
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: SUCCESS (Legendary Resistance) — Polymorph resisted!`)
        return { state: currentState, logs }
      }
    }

    if (saveResult.success) {
      logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] SUCCESS — resists Polymorph!`)
      return { state: currentState, logs }
    }

    logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] FAIL!`)

    // Transform into sheep — Polymorph replaces ALL game statistics
    ...
    logs.push(`  → ${target.name} is polymorphed into a ${sheepForm.name}! (${sheepForm.maxHP} HP, AC ${sheepForm.ac || 10})`)
  ...
  logs.push(`  → ${target.name} polymorphs into a ${beastForm.name}! (${beastForm.maxHP} HP, AC ${beastForm.ac || 13}, multiattack: ${beastForm.multiattack || 0})`)
 ...
 */
function resolveSleepSpell(state, casterId, spellDef, slotLevel, targets, parentLogs) {
  const logs = []
  let currentState = state
  ...
  logs.push(`  → Sleep HP pool: ${poolRoll.total} (${baseDice}d8)`)
  ...
  for (const target of sortedTargets) {
    if (target.currentHP <= remainingPool) {
      ...
      sleptCount++
      logs.push(`    → ${target.name} (${target.currentHP} HP) falls asleep! Pool remaining: ${remainingPool}`)
    } else {
      logs.push(`    → ${target.name} (${target.currentHP} HP) — not enough pool remaining (${remainingPool})`)
      break
    }
  }

  if (sleptCount === 0) {
    logs.push(`  → No creatures affected by Sleep`)
  }
...
function resolveSelfSpell(state, casterId, spellDef, slotLevel, parentLogs) {
  ...
  // Apply self-targeted buff effects (invisible, teleport, globe_of_invulnerability, etc.) — BUG-2 fix
  if (spellDef.selfEffects && spellDef.selfEffects.length > 0) {
    ...
    logs.push(`  → ${caster.name} gains: ${spellDef.selfEffects.join(', ')}`)
  }
...
function applySpellBuff(state, targetId, spellDef, logs) {
  ...
  for (const effect of spellDef.selfEffects) {
    if (effect === 'ac_bonus_2') {
      ...
      logs.push(`    → ${target.name} gains +2 AC (${target.ac} → ${updates.ac})`)
    } else if (effect === 'ac_set_13_plus_dex') {
      const newAC = 13 + (target.dexMod || 0)
      updates.ac = newAC
      logs.push(`    → ${target.name} gains Mage Armor: AC set to ${newAC} (13 + DEX ${target.dexMod || 0})`)
    } else if (effect === 'ac_bonus_5') {
      updates.ac = (target.ac || 0) + 5
      logs.push(`    → ${target.name} gains +5 AC (${target.ac} → ${updates.ac})`)
    } else {
  ...
  if (conditionsToAdd.length > 0) {
    ...
    logs.push(`    → ${target.name} gains: ${conditionsToAdd.join(', ')}`)
  }
...
function applySpellDamage(state, casterId, targetId, spellDef, slotLevel, isCrit = false, multiplier = 1) {
  ...
  const caster = state.getCombatant(casterId)
  // Resolve 'casting_mod' string to actual caster modifier (Spiritual Weapon, etc.) — BUG-7 fix
  ...
  // Bonus dice for spells with combined damage types (Ice Storm: 2d8 bludgeoning + 4d6 cold) — BUG-6 fix
  ...
  if (damage < rawDamage) {
    sideEffectLogs.push(`    → ${target.name} resists ${spellDef.damage.type} damage (${rawDamage} → ${damage})`)
  }
...
function applySpellEffects(state, targetId, spellDef, logs) {
  ...
  if (hasCharmEffect && target.immuneCharmed) {
    logs.push(`    → ${target.name} is immune to charm — effect blocked!`)
    return state
  }
  ...
  if (conditionsAdded > 0) {
    logs.push(`    → ${target.name} gains: ${spellDef.effects.join(', ')}`)
...
function checkConcentration(state, targetId, damage) {
  ...
  if (conSave.success) {
    logs.push(`    → Concentration save: [${conSave.total} vs DC ${conSave.dc}] MAINTAINED!`)
    return { state, logs }
  } else {
    logs.push(`    → Concentration save: BROKEN! ${target.concentrating} ends!`)
    return breakConcentration(state, targetId)
  }
}

function breakConcentration(state, casterId) {
  const caster = state.getCombatant(casterId)
  if (!caster.concentrating) return { state, logs: [] }

  const spell = caster.concentrating
  const logs = [`  → ${caster.name} loses concentration on ${spell}`]
    ...
    switch (spell) {
      ...
      case 'Polymorph': {
        ...
        if (newConditions.includes('polymorphed') && c.prePolymorphState) {
          ...
          logs.push(`  → ${c.name} reverts from Polymorph to original form (HP: ${pre.currentHP}/${pre.maxHP})`)
...
function resolveReactionMovement(state, casterId, targetId, logs) {
  ...
  if (target.reactedThisRound) {
    logs.push(`    → ${target.name} has no reaction available — cannot move away`)
  ...
  if (tPos.q != null || tPos.r != null) {
    ...
    // Normalize direction — pick the dominant hex axis
  ...
  logs.push(`    → ${target.name} uses its reaction to move away: ${posStr(tPos)} → ${posStr(newPos)} [${speed}ft]`)
...
function resolveMove(state, combatantId, choice) {
  ...
  const dist = TurnMenu._gridDistance(from, to)
  ...
  if (!actor.disengaged) {
    ...
    for (const opp of opponents) {
      const distBefore = TurnMenu._gridDistance(from, opp.position)
      const distAfter  = TurnMenu._gridDistance(to,   opp.position)
 ...
 */
function resolveBreathWeapon(state, combatantId, choice) {
  const actor = state.getCombatant(combatantId)
  ...
  logs.push(`  → Breath Weapon hits ${targets.length} target(s)`)
  ...
  for (const virtualTarget of targets) {
    ...
    if (saveResult.success) {
      ...
      logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] SUCCESS — ${halfDmg} damage (half)${resistNote}`)
      ...
      if (saved) {
        ...
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: SUCCESS (Legendary Resistance) — ${halfDmg} damage (half)`)
      } else {
        ...
        logs.push(`  → ${target.name} ${saveAbility.toUpperCase()} save: [${saveResult.total} vs DC ${dc}] FAIL — ${fullDmg} damage${resistNote}`)
...
function resolveDragonFear(state, combatantId, choice) {
  ...
  const targets = resolveAoETargets(virtualCaster, spellLikeShape, choice.aoeCenter, virtualCombatants)
  logs.push(`  → Dragon Fear targets ${targets.length} creature(s)`)
  ...
  for (const virtualTarget of targets) {
    ...
    if (target.immuneFrightened) {
      logs.push(`  → ${target.name} is immune to frightened — no effect!`)
      continue
    }
    ...
    if (saveResult.success) {
      logs.push(`  → ${target.name} WIS save: [${saveResult.total} vs DC ${dc}] SUCCESS — resists fear!`)
    } else {
      // Check Legendary Resistance
      let saved = false
      const lr = tryLegendaryResistance(currentState, virtualTarget.id, logs)
      ...
      if (lr.used) {
        saved = true
        logs.push(`  → ${target.name} WIS save: SUCCESS (Legendary Resistance) — resists fear!`)
      }
      ...
      if (!saved) {
        ...
        frightenedCount++
        logs.push(`  → ${target.name} WIS save: [${saveResult.total} vs DC ${dc}] FAIL — frightened!`)
      }
    }
  }
 ...
 * @returns {{ state: GameState, result: object }}
 */
function resolveLootCorpse(state, combatantId, option) {
  const actor = state.getCombatant(combatantId)
  ...
  if (loot.items && loot.items.length > 0) {
    for (const item of loot.items) {
      lootDescParts.push(`${item.quantity}× ${item.itemId}`)
    }
  }
  if (loot.currency) {
...
module.exports = {
  resolve,
  // Exported for testing individual resolvers
  _resolveAttack: resolveAttack,
  _resolveMultiattack: resolveMultiattack,
  _resolveSpell: resolveSpell,
  _resolveBreathWeapon: resolveBreathWeapon,
  _resolveDragonFear: resolveDragonFear,
  _resolveLootCorpse: resolveLootCorpse,
  _breakConcentration: breakConcentration,
  _checkPolymorphRevert: checkPolymorphRevert,
  _resolveSleepSpell: resolveSleepSpell,
  _resolvePolymorphSpell: resolvePolymorphSpell,
}
