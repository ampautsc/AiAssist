/**
 * StepResolver — Step-by-step action resolution with dice requests.
 *
 ...
 *   3. Returns { pendingDice, displayDice, phase, ... } — engine pauses
 ...
 *   7. Returns final { state, result } — no more dice needed
 *
 * This module does NOT touch the session layer — it operates purely on
 * GameState + choice → steps. The CombatSessionManager orchestrates the
...
// ── Step Context ─────────────────────────────────────────────────────────────
// Each in-flight action is tracked as a StepContext — a serializable object
 ...
 * @returns {StepResult}
 */
function beginResolve(state, combatantId, choice) {
  ...
  // Actions that need no dice — resolve immediately via ActionResolver  
  ...
  // Dice-requiring actions — build the first step
  switch (option.type) {
    case 'attack':
    ...
    default:
      // Fallback: anything we haven't step-ified yet — resolve synchronously
      const resolution = ActionResolver.resolve(state, combatantId, choice)
      return {
        done: true,
        state: resolution.state,
 ...
 * @returns {StepResult}
 */
function continueResolve(context, diceResult) {
...
function _continueAttack(context, diceResult) {
  ...
  if (context.phase === 'damage') {
    ...
    // No concentration save needed — we're done
    newState = _finalizeMultiattackAction(newState, attackerId)
  ...
  if (context.phase === 'concentration_save') {
    ...
    if (!success) {
      ...
      logs.push(`  💔 ${updatedTarget.name} fails concentration save (${total} vs DC ${conSaveDC}) — ${updatedTarget.concentrating} ends!`)
    } else {
      logs.push(`  ✅ ${updatedTarget.name} maintains concentration (${total} vs DC ${conSaveDC})`)
    }
...
// ── Multiattack Resolution (stepped) ─────────────────────────────────────────
...
function _continueMultiattack(context, diceResult) {
  ...
  if (context.phase === 'attack_roll') {
    ...
    if (!hits) {
      // Miss — check if more attacks remain
      ...
      if (currentAttack < attackCount && target.currentHP > 0) {
        // More attacks — request next attack roll
  ...
  if (context.phase === 'multi_damage') {
    ...
    // Concentration check — request a save if needed
    const updatedTarget = newState.getCombatant(targetId)
    if (updatedTarget.concentrating && updatedTarget.currentHP > 0) {
      const dc = Math.max(10, Math.floor(damageTotal / 2))
      const saveBonus = updatedTarget.saves ? (updatedTarget.saves.con || 0) : 0
    ...
    // No con save — continue to next attack or finalize
  ...
  if (context.phase === 'multi_con_save') {
    ...
    if (!success) {
      ...
      logs.push(`  💔 ${updatedTarget.name} fails concentration save — ${updatedTarget.concentrating} ends!`)
    } else {
      logs.push(`  ✅ ${updatedTarget.name} maintains concentration`)
    }
...
// ── Spell Resolution (stepped) ───────────────────────────────────────────────

function _beginSpell(state, casterId, option, choice) {
  // Spells are complex — many subtypes.
  // For now, we handle the most common patterns:
  // - Spell attack (d20 → damage)
  // - Save-based (targets make saves → damage on fail)
  ...
  // Self-buff, utility spells, polymorphs, etc. — resolve synchronously for now
...
function _continueSpell(context, diceResult) {
  // For spell attack → damage → concentration save chain,
  // and for save-based spells → damage chain.
  ...
  // Fallback — resolve synchronously
  const state = _deserializeState(context.state)
...
function _continueSpellAttack(context, diceResult) {
  ...
  if (!hits) {
    // Spell misses — still consume the spell slot/action
  ...
  if (!match) {
    // Can't parse damage dice — fall back to synchronous
    const resolution = ActionResolver.resolve(state, context.actorId, context.choice)
    return {
      done: true,
      state: resolution.state,
...
function _continueSpellDamage(context, diceResult) {
  // Spell damage resolved — apply via ActionResolver for full effect
...
// ── Breath Weapon Resolution (stepped) ───────────────────────────────────────
...
function _beginBreathWeapon(state, actorId, option, choice) {
  ...
  if (!breathDamage) {
    // No breath weapon data — fall back
...
function _continueBreathWeapon(context, diceResult) {
  ...
  // For initial release: resolve via ActionResolver (saves are enemy rolls → auto)
  const state = _deserializeState(context.state)
  const resolution = ActionResolver.resolve(state, context.actorId, context.choice)
...
function _computeAdvantage(attacker, target, state) {
...
function _isForceCrit(attacker, target, state) {
  const isTargetParalyzed = (target.conditions || []).includes('paralyzed')
  const within5ft = TurnMenu._combatDistance(attacker, target) <= 5
...
function _interpretD20(rolls, advDisadv) {
...
function _finalizeMultiattackAction(state, attackerId) {
  ...
  if (typeof ActionResolver._finalizeMultiattackAction === 'function') {
    return ActionResolver._finalizeMultiattackAction(state, attackerId)
  }
...
function _checkPolymorphRevert(state, targetId, overflowDamage) {
  ...
  const logs = [`  🔄 ${target.name} reverts from beast form!`]
...
function _removeCharmedOnDamage(state, targetId, damage) {
  ...
  return {
    state: newState,
    logs: [`  ✨ ${target.name} is freed from the charm!`],
  }
}

/** Extract dice display info from a synchronous ActionResolver result */
function _extractDiceFromResult(result) {
  const diceReqs = []
