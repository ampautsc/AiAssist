 ...
 *
 * Each creature template key maps to a loot table describing possible drops.
 * Used by LootService to generate loot when a creature dies.
 *
 * Loot entry shapes:
 *   Item:     { type: 'item',     itemId: string, chance: 0-1, quantity: number|string }
 *   Currency: { type: 'currency', currency: string, chance: 0-1, amount: number|string }
 *
 * `quantity` and `amount` can be a fixed number or a dice expression string (e.g. '2d6').
 * `chance` is the probability (0-1) that this entry drops.
 *
 * Templates without a loot table entry here get a default "no loot" result.
 */
...
/**
 * Get the loot table for a creature template key.
 * @param {string} templateKey - Creature template key
 * @returns {Array} Loot table entries, or empty array if none defined
 */
function getLootTable(templateKey) {
  return LOOT_TABLES[templateKey] || []
}

/**
 * Check if a creature template has a loot table.
 * @param {string} templateKey
 * @returns {boolean}
 */
function hasLootTable(templateKey) {
  return templateKey in LOOT_TABLES
}

/**
 * Get all template keys that have loot tables defined.
 * @returns {string[]}
 */
function getLootTableKeys() {
