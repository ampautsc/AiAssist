 ...
 *
 * Usage:
 *   const { createCreature, getTemplateKeys } = require('./creatures')
 ...
 *
 * Each template produces a fully initialised combat object including:
 *   currentHP, maxHP, ac, saves, conditions, concentrating, flying, weapons, etc.
 */
...
CREATURE_TEMPLATES['cult_fanatic'] = {
  ...
  features: {
    darkDevotion: true,
    spiritualWeapon: null,   // starts null, set when cast
    spellcasting: {
      saveDC:       11,
      attackBonus:   3,
      slots:  { 1: 4, 2: 3 },
...
// Minimal legacy stat blocks for the old encounterRunner (uses different schema)
const CREATURES = {
  goblin: {
...
function getCreature(id) { return CREATURES[id] ?? null }
function listCreatures() { return Object.keys(CREATURES) }
