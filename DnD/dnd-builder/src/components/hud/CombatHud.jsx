...
function computeToolbar(hexPts) {
  ...
  const [V2x]      = hexPts[2]
  const w = V1x - V2x - TB_INSET * 2
  const x = V2x + TB_INSET
  const y = Math.max(4, (V1y - TB_H) / 2)
  return { x, y, w, h: TB_H, r: TB_R }
}

/** Portrait circle on the outward normal of V2→V3, Cy pinned near top. */
...
export default function CombatHud({
  ...
  const btnW  = (tb.w - btnPad * 2 - btnGap * (TB_BTNS.length - 1)) / TB_BTNS.length
  const btnH  = tb.h - 8
  const btnY0 = tb.y + 4
  ...
  return (
    <div className="hud" data-server-menu={serverMenu ? 'ready' : 'loading'}>
      ...
      >
        ...
        {(budget || serverMenu) && TB_BTNS.map((def, i) => {
          const x   = bx(i)
          ...
          return (
            ...
            >
              <rect x={x} y={btnY0} width={btnW} height={btnH} rx={6}
                fill={dis ? '#1a1612' : act ? '#3a3020' : '#252018'}
                stroke={dis ? '#333' : act ? '#c8a040' : '#5a4830'}
                strokeWidth={act ? 2 : 1}
              />
              <text
                x={x + btnW / 2} y={btnY0 + btnH / 2}
                textAnchor="middle" dominantBaseline="central"
