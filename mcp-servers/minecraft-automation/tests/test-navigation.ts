...
async function callTool(toolName: string, args: any = {}): Promise<string> {
  ...
  // Call the MCP server via stdio - from build/tests/, go up to build/, then to index.js
  const serverPath = path.join(__dirname, '..', 'index.js');
  const { stdout } = await execAsync(
    `echo '${input}' | node "${serverPath}"`,
...
async function runTests() {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      // From build/tests/ we need to go: ../.. to mcp-servers/minecraft-automation, then ../.. to workspace root
      const dataPath = path.join(__dirname, '..', '..', '..', '..', 'minecraft-navigation-data');
      const beforePath = path.join(dataPath, test.beforeScreenshot);
      const afterPath = path.join(dataPath, test.afterScreenshot);
